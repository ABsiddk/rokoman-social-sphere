...
const PhoneSecuritySection: React.FC<Props> = ({ isEditing }) => {
  ...
  return (
    <div>
      <h3 className="font-semibold text-lg mb-4 text-gray-800 dark:text-cyan-100">{t("profile.phoneSecurity")}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <LiquidGlassInput
            ...
            className="w-full"
            placeholder={isEditing ? t("profile.phonePlaceholder") : (currentUser?.phone || t("profile.phoneEmpty"))}
            autoComplete="off"
            disabled={!isEditing}
          />
        </div>
